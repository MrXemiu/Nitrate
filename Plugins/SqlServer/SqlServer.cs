...
namespace Nitrate.Plugins.SqlServer
{
    ...
    public class SqlServer : BasePlugin<SqlServerConfig>
    {
        ...
        private static void EnsureUser(SqlServerConfig config, Server server, Database db)
        {
            ...
            var user = GetDbUser(config, db);

            EnsureUserHasDbOwnerRole(user);

            EnsureUserSchema(config, user);

            EnsureUserIsDbOwner(config, db);
        }

        private static void EnsureUserIsDbOwner(SqlServerConfig config, Database db)
        {
            if (!db.Owner.Equals(config.DbUser, StringComparison.OrdinalIgnoreCase))
                db.SetOwner(config.DbUser, true);
        }

        private static void EnsureUserSchema(SqlServerConfig config, User user)
        {
        ...
        private static void EnsureUserHasDbOwnerRole(User user)
        {
            if (!user.EnumRoles().Contains("db_owner"))
                user.AddToRole("db_owner");
        }

        private static User GetDbUser(SqlServerConfig config, Database db)
        {
            var user = db.UserForLogin(config.DbUser);
            if (user != null) return user;

            user = new User(db, config.DbUser) {Login = config.DbUser};
            user.Create();
            return user;
        }
